...
struct RtcStats
{
  ...
  RtcStats()
      ...
      , txVideoKBitRate(0)
      , lastmileDelay(0)
      , txPacketLossRate(0), rxPacketLossRate(0), userCount(0), cpuAppUsage(0), cpuTotalUsage(0),
    ...
    */
    ...
    VideoEncoderConfiguration()
        ...
        , bitrate(STANDARD_BITRATE), minBitrate(DEFAULT_MIN_BITRATE),
          orientationMode(ORIENTATION_MODE_ADAPTIVE), degradationPreference(MAINTAIN_QUALITY),
...
struct RtcEngineContext
{
    ...
    int areaCode;
    RtcEngineContext()
            : eventHandler(NULL), appId(NULL), context(NULL), areaCode(rtc::AREA_CODE_GLOBAL)
    {}
};
...
public:
    ...
    int setLocalVoiceChanger(VOICE_CHANGER_PRESET voiceChanger) {
        if (voiceChanger == 0x00000000) {
            ...
            return m_parameter ? m_parameter->setInt("che.audio.morph.voice_changer",
                                                     static_cast<int>(voiceChanger - 0x00100000 +
                                                                      6)) : -ERR_NOT_INITIALIZED;
    ...
    int setLogFilter(unsigned int filter) {
        return m_parameter ? m_parameter->setUInt("rtc.log_filter", filter & LOG_FILTER_MASK)
                           : -ERR_NOT_INITIALIZED;
    }
    ...
    int setRemoteRenderMode(uid_t uid, RENDER_MODE_TYPE renderMode) {
        return setParameters("{\"che.video.render_mode\":[{\"uid\":%u,\"renderMode\":%d}]}", uid,
                             renderMode);
    }
